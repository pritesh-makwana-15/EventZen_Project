    ...
    public class AuthServiceImpl implements AuthService {
        ...
        public AuthResponse register(RegisterRequest request) throws Exception {
            ...
            // 🔧 Handle image URL safely - truncate or skip if too long
            String imageUrl = request.getProfileImage();
            if (StringUtils.hasText(imageUrl)) {
                // If image URL is too long (likely base64), skip it for now
                if (imageUrl.length() > 1000) {
                    System.out.println("⚠️ Image URL too long, skipping for registration: " + imageUrl.length() + " characters");
